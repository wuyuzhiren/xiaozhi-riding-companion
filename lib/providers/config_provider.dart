...
class ConfigProvider extends ChangeNotifier {
  ...
  List<OpenAIConfig> _openaiConfigs = [];
  ...
  List<OpenAIConfig> get openaiConfigs => _openaiConfigs;
  OpenAIConfig? get openaiConfig =>
      _openaiConfigs.isNotEmpty ? _openaiConfigs.first : null;
  ...
  Future<void> _loadConfigs() async {
    ...
    // Load OpenAI-compatible configs
    final openaiConfigsJson = prefs.getStringList('openaiConfigs') ?? [];
    _openaiConfigs =
        openaiConfigsJson
            .map((json) => OpenAIConfig.fromJson(jsonDecode(json)))
            .toList();
  ...
  Future<void> _saveConfigs() async {
    ...
    // Save OpenAI-compatible configs
    final openaiConfigsJson =
        _openaiConfigs.map((config) => jsonEncode(config.toJson())).toList();
    await prefs.setStringList('openaiConfigs', openaiConfigsJson);
  }

  // Add OpenAI-compatible config
  Future<void> addOpenAIConfig(
    String name,
    String apiUrl,
    String apiKey,
    String model, {
    String ttsModel = 'FunAudioLLM/CosyVoice2-0.5B',
    String ttsVoice = 'FunAudioLLM/CosyVoice2-0.5B:claire',
    ...
    _openaiConfigs.add(newConfig);
    await _saveConfigs();
    notifyListeners();
  }

  Future<void> updateOpenAIConfig(OpenAIConfig updatedConfig) async {
    final index = _openaiConfigs.indexWhere(
      (config) => config.id == updatedConfig.id,
    );
    if (index != -1) {
      _openaiConfigs[index] = updatedConfig;
      await _saveConfigs();
      notifyListeners();
    }
  }

  Future<void> deleteOpenAIConfig(String id) async {
    _openaiConfigs.removeWhere((config) => config.id == id);
    await _saveConfigs();
    notifyListeners();
  }
