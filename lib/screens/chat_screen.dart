...
import 'package:ai_assistant/screens/live2d_screen.dart';
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  OpenAIService? _openaiService; // OpenAI兼容服务
  OpenAIConfig? _openaiConfig;
  final AudioPlayer _ttsPlayer = AudioPlayer();
  Timer? _mouthTimer;
  final GlobalKey<Live2DScreenState> _live2dKey =
      GlobalKey<Live2DScreenState>();
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (widget.conversation.type == ConversationType.xiaozhi) {
        ...
        // 初始化 MiniMaxService
        _initMiniMaxService();
      } else if (widget.conversation.type == ConversationType.openai) {
        // 初始化 OpenAI 兼容服务
        _initOpenAIService();
      }
    });
  ...
  /// 生成并播放语音回复，同时驱动 Live2D 口型。
  Future<void> _speakReply(String text) async {
    final config = _openaiConfig;
    if (config == null) return;
    try {
      final dir = await getTemporaryDirectory();
      final path =
          '${dir.path}/tts_${DateTime.now().millisecondsSinceEpoch}.mp3';
      final file = await OpenAIService.tts(
      ...
      // 口型驱动：播放期间以波形近似驱动嘴巴开合
      var tick = 0;
      _mouthTimer?.cancel();
      _mouthTimer = Timer.periodic(const Duration(milliseconds: 80), (t) {
        tick++;
        final v = 0.25 +
            0.55 *
                (0.5 +
                    0.5 *
                        math.sin(tick * 0.65) *
                        math.sin(tick * 0.23));
        _live2dKey.currentState?.setMouth(v.clamp(0.0, 1.0));
      });
      ...
      _mouthTimer?.cancel();
      _live2dKey.currentState?.setMouth(0.0);
      _live2dKey.currentState?.setSpeaking(false);
    } catch (e) {
      print('语音回复失败: $e');
      _mouthTimer?.cancel();
      _live2dKey.currentState?.setMouth(0.0);
  ...
  void _initOpenAIService() {
    ...
    if (configId != null && configId.isNotEmpty) {
      openaiConfig =
          configProvider.openaiConfigs
              .where((config) => config.id == configId)
              .firstOrNull;
    }

    if (openaiConfig == null) {
      if (configProvider.openaiConfigs.isEmpty) {
        throw Exception("未设置大模型API配置，请先在设置中配置 SiliconFlow 等");
      }
      openaiConfig = configProvider.openaiConfigs.first;
    }
      ...
      appBar: AppBar(
        ...
        actions: [
          if (widget.conversation.type == ConversationType.dify ||
              ...
              widget.conversation.type == ConversationType.openai)
            IconButton(
              icon: const Icon(Icons.refresh, color: Colors.black, size: 24),
              tooltip: '开始新对话',
              onPressed: _resetConversation,
            ),
          if (widget.conversation.type == ConversationType.openai)
            IconButton(
              icon: const Icon(
  ...
  void _resetConversation() async {
    ...
    if (widget.conversation.type == ConversationType.minimax) {
      ...
      final sessionId = widget.conversation.id;
      _openaiService?.clearConversation(sessionId);
  ...
  void _sendMessage() async {
    ...
    try {
      if (widget.conversation.type == ConversationType.minimax) {
        ...
        await _openaiService!.streamChat(
          message,
          sessionId: sessionId,
          onDelta: (delta) {
            buffer.write(delta);
          ...
          onDone: (full) async {
            if (!mounted) return;
            final reply = full.isEmpty ? buffer.toString() : full;
            await conversationProvider.addMessage(
              conversationId: widget.conversation.id,
              role: MessageRole.assistant,
              content: reply,
            );
            if (reply.isNotEmpty && _openaiConfig?.voiceReply == true) {
              _speakReply(reply);
            }
          },
          onError: (err) async {
