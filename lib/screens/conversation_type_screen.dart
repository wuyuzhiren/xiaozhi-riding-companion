...
class _ConversationTypeScreenState extends State<ConversationTypeScreen> {
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (_showOpenAISelector) ...[
                      const SizedBox(height: 16),
                      _buildOpenAISelectionCard(),
                    ],
                  ],
                ),
              ),
          ...
          InkWell(
            onTap: () {
              ...
              if (openaiConfigs.isEmpty) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('请先在设置中添加大模型API配置（SiliconFlow等）')),
                );
                return;
              }

              setState(() {
                _selectedType = ConversationType.openai;
                _showXiaozhiSelector = false;
                _showDifySelector = false;
                _showMiniMaxSelector = false;
                _showOpenAISelector = true;
                _selectedOpenAIConfig =
                    openaiConfigs.isNotEmpty ? openaiConfigs.first : null;
              });
            },
            child: Container(
              margin: const EdgeInsets.only(
                ...
                border: Border.all(
                  color:
                      _selectedType == ConversationType.openai
                          ? Colors.black
                          : Colors.grey.shade300,
                  width: _selectedType == ConversationType.openai ? 2 : 1,
                ),
                boxShadow: [
                  BoxShadow(
                    color: Colors.black.withOpacity(
                      _selectedType == ConversationType.openai ? 0.08 : 0.03,
                    ),
                    blurRadius:
                        _selectedType == ConversationType.openai ? 6 : 3,
                    spreadRadius: 0,
                    offset: const Offset(0, 2),
                  ),
                ],
  ...
  Widget _buildOpenAISelectionCard() {
    final openaiConfigs = Provider.of<ConfigProvider>(context).openaiConfigs;
        ...
        children: [
          ...
          ...openaiConfigs.map(
            (config) => InkWell(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color:
                      _selectedOpenAIConfig?.id == config.id
          ...
          if (openaiConfigs.isEmpty)
            Padding(
              ...
              child: Center(
                child: Text(
                  '还没有配置，请到"设置"中添加 SiliconFlow 等大模型API',
  ...
  void _createConversation() async {
    ...
    } else if (_selectedType == ConversationType.openai &&
        _selectedOpenAIConfig != null) {
      _createOpenAIConversation(_selectedOpenAIConfig!);
    }
  }
  ...
  void _createOpenAIConversation(OpenAIConfig config) async {
    final conversation = await Provider.of<ConversationProvider>(
      context,
      listen: false,
    ).createConversation(
      title: '与 ${config.name} 的对话',
      type: ConversationType.openai,
      configId: config.id,
    );
